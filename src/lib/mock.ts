export const BREEDS = [
  { jp: "Indian Pariah Dog", en: "Indian Pariah Dog", rank: 1, size: "medium" },
  { jp: "Labrador Retriever", en: "Labrador Retriever", rank: 2, size: "large" },
  { jp: "Golden Retriever", en: "Golden Retriever", rank: 3, size: "large" },
  { jp: "Indian Spitz", en: "Indian Spitz", rank: 4, size: "small" },
  { jp: "Pomeranian", en: "Pomeranian", rank: 5, size: "small" },
  { jp: "Shih Tzu", en: "Shih Tzu", rank: 6, size: "small" },
  { jp: "Beagle", en: "Beagle", rank: 7, size: "medium" },
  { jp: "Rajapalayam", en: "Rajapalayam", rank: 8, size: "large" },
  { jp: "German Shepherd", en: "German Shepherd", rank: 9, size: "large" },
  { jp: "Mixed", en: "Mixed", rank: 10, size: "medium" },
];

export const DAILY_FACTS = [
  { jp: "Dogs have unique nose prints, just like human fingerprints.", en: "Dogs have unique nose prints, just like human fingerprints." },
  { jp: "The Indian Pariah Dog is one of the world's oldest naturally evolved breeds — hardy, smart and perfectly suited to India's climate.", en: "The Indian Pariah Dog is one of the world's oldest naturally evolved breeds — hardy, smart and perfectly suited to India's climate." },
  { jp: "A dog's normal body temperature is 38.3–39.2°C, higher than humans.", en: "A dog's normal body temperature is 38.3–39.2°C, higher than humans." },
  { jp: "Dogs can distinguish around 10,000 different scents.", en: "Dogs can distinguish around 10,000 different scents." },
  { jp: "Dogs dream too — they twitch during REM sleep.", en: "Dogs dream too — they twitch during REM sleep." },
];

export const CLINICS = [
  { jp: "Bandra Pet Hospital", en: "Bandra Pet Hospital", rating: 4.6, km: 0.8, open: true, em: true },
  { jp: "Juhu Veterinary Clinic", en: "Juhu Veterinary Clinic", rating: 4.4, km: 1.2, open: true, em: false },
  { jp: "Andheri 24x7 Animal Medical", en: "Andheri 24x7 Animal Medical", rating: 4.8, km: 2.1, open: true, em: true },
  { jp: "Powai Pet Wellness Centre", en: "Powai Pet Wellness Centre", rating: 4.7, km: 3.4, open: true, em: false },
  { jp: "Colaba Animal Hospital", en: "Colaba Animal Hospital", rating: 4.2, km: 5.6, open: false, em: false },
];

export const POSTS = [
  { id: "1", user: "Priya & Bruno", breed: "Indian Pariah Dog", time: "2h ago", titleJp: "My Indie's temp seems high", titleEn: "My Indie's temp seems high", flair: "健康", up: 47, com: 12, location: "Bandra, Mumbai" },
  { id: "2", user: "Mumbai Animal Lover", breed: "Indian Spitz", time: "1h ago", titleJp: "Recommended vet in Mumbai?", titleEn: "Recommended vet in Mumbai?", flair: "獣医Q&A", up: 23, com: 34, location: "Andheri, Mumbai" },
  { id: "3", user: "Arjun's Pack", breed: "Labrador Retriever", time: "Today", titleJp: "Our daily walk routine at Marine Drive", titleEn: "Our daily walk routine at Marine Drive", flair: "日常", up: 89, com: 6, location: "Marine Drive, Mumbai" },
  { id: "4", user: "Lost Pet Support", breed: "Mixed Breed", time: "2h ago", titleJp: "Did you see a black Indie near Bandra station?", titleEn: "Did you see a black Indie near Bandra station?", flair: "迷子", up: 156, com: 28, location: "Bandra, Mumbai" },
];

